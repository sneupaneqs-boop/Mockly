...
import { useTopicPerformance, calcStreak } from '../lib/useTopicPerformance'
...
export default function History() {
  ...
  const { topics } = useTopicPerformance(user?.id)
  ...
  const stats = useMemo(() => {
    ...
    const streak = calcStreak(mocks)
    return { total: mocks.length, avg, best, trend, lastScores, streak }
  }, [mocks])

  // Heatmap cells (26 weeks × 7 days) — only real mock dates highlighted
  const cells = useMemo(() => {
    const today = new Date()
    const mockDates = new Set(mocks.filter(m => m.completed_at).map(m => m.completed_at?.slice(0, 10)))
    return Array.from({ length: 182 }, (_, i) => {
      const d = new Date(today); d.setDate(d.getDate() - (181 - i))
      const iso = d.toISOString().slice(0, 10)
      const level = mockDates.has(iso) ? 4 : 0
      return { iso, level }
    })
  }, [mocks])

  const weakTopics = useMemo(() =>
    topics.filter(t => t.total > 0).sort((a, b) => (a.strength ?? 100) - (b.strength ?? 100)).slice(0, 5),
    [topics]
  )
  ...
  // Topics with real strength for radar (need at least attempted)
  const radarTopics = useMemo(() =>
    topics.filter(t => t.total > 0).slice(0, 8).map(t => ({ ...t, strength: t.strength ?? 0 })),
    [topics]
  )
    ...
    <div className="page page-enter">
      ...
      <div className="kpi-grid">
        <div className="kpi accent">
          ...
          <Sparkline values={stats.total > 0 ? [stats.total] : [0]} color="#5B5BFF"/>
        </div>
        <div className="kpi green">
          ...
          <div className={`k-trend ${stats.trend < 0 ? 'down' : ''}`}>
            {stats.lastScores.length > 1 ? `${stats.trend >= 0 ? '▲' : '▼'} ${Math.abs(stats.trend)} pts last 5` : 'Not enough data'}
          </div>
        ...
        <div className="kpi warm">
          ...
          <Sparkline values={stats.lastScores.length ? [...stats.lastScores].reverse() : [0]} color="#FF7A45"/>
        </div>
        <div className="kpi violet">
          <div className="k-label">Current streak</div>
          <div className="k-val">{stats.streak}<span style={{ fontSize: 18, opacity: .5 }}>d</span></div>
          <div className="k-trend">{stats.streak > 0 ? '🔥 Keep going' : 'Complete a mock today'}</div>
          <Sparkline values={[stats.streak || 0]} color="#8B5CF6"/>
        </div>
      </div>
            ...
            <div className="chart-card">
              <div className="chart-head">
                <div>
                  ...
                  <p className="chart-sub">Top {radarTopics.length} attempted topics</p>
                </div>
              </div>
          ...
          <div className="chart-card" style={{
            ...
            <div className="chart-head" style={{ position: 'relative' }}>
              <div>
                ...
                <p className="chart-sub" style={{ color: 'rgba(255,255,255,.6)' }}>Based on your performance</p>
              </div>
                ...
                <div key={t.num} style={{ display: 'flex', alignItems: 'start', gap: 14, padding: 14, borderRadius: 14, background: 'rgba(255,255,255,.05)', border: '1px solid rgba(255,255,255,.08)' }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: 12.5, color: 'rgba(255,255,255,.6)', lineHeight: 1.5 }}>
                      {t.strength}% accuracy · {t.total} questions answered.
                    </div>
      ...
      <div className="chart-card" style={{ marginTop: 18 }}>
        <div className="chart-head">
          <div>
            ...
            <p className="chart-sub">Last 26 weeks — completed mocks highlighted</p>
          </div>
          {stats.streak > 0 && <span className="tag strong"><Icons.flame size={11}/> {stats.streak}-day streak</span>}
        </div>
